...
export function FaqAccordion() {
  ...
  return (
    <section className="py-[80px] md:py-[110px] bg-white border-t border-[#EDEFF2]">
      <div className="container mx-auto px-6 sm:px-8 md:px-4 max-w-3xl">
        <h2 className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-[800] font-heading text-[#0F172A] mb-8 md:mb-10 text-center">
          Frequently asked questions
        </h2>

        <Accordion type="single" collapsible className="w-full px-1 sm:px-0">
          {faqs.map((faq, idx) => (
            <AccordionItem key={idx} value={`item-${idx}`} className="border-b border-[#EDEFF2] py-1 md:py-2">
              <AccordionTrigger className="text-left font-[700] font-heading text-sm sm:text-base md:text-lg text-[#0F172A] hover:text-primary hover:no-underline transition-colors gap-4 py-3 md:py-4 [&>svg]:text-primary [&>svg]:w-4 [&>svg]:h-4 md:[&>svg]:w-5 md:[&>svg]:h-5">
                {faq.q}
              </AccordionTrigger>
              <AccordionContent className="text-[#4B5563] font-body text-sm md:text-base leading-relaxed pr-2 md:pr-0">
                {faq.a}
              </AccordionContent>
            </AccordionItem>
          ))}
